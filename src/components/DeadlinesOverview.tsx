...
export function DeadlinesOverview({
    ...
    // Live-Timer für den echten Countdown
    const [now, setNow] = useState(new Date());

    useEffect(() => {
        const interval = setInterval(() => setNow(new Date()), 60000); // Jede Minute aktualisieren
                                            ...
                                            <div className="flex items-center justify-between gap-2 mb-1">
                                                ...
                                                {(() => {
                                                    ...
                                                    if (diffDays === 0) {
                                                        ...
                                                        // Wenn wir eine Uhrzeit haben, rechnen wir die genauen Minuten aus
                                                        if (deadline.time) {
                                                            const [hours, minutes] = deadline.time.split(":").map(Number);
                                                            const deadlineExact = new Date(year, month - 1, day, hours, minutes);

                                                            const diffMs = deadlineExact.getTime() - now.getTime();
                                                            ...
                                                            if (diffMins < 0) {
                                                                ...
                                                                timeStr = "Jetzt fällig";
                                                            } else if (diffMins < 60) {
                                                                timeStr = `In ${diffMins} Min`;
                                                            } else {
                                                                const h = Math.floor(diffMins / 60);
                                                                const m = diffMins % 60;
                                                                timeStr = m > 0 ? `In ${h}h ${m}m` : `In ${h}h`;
                                                            }
                                                        }
                                                    }
                                                    else if (diffDays === 1) {
